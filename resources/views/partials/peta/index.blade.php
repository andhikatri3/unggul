...
@section('content')
    <div>
        <div class="row">
            <div class="col-md-12" style="position:relative;">
                <div id="map-loading" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:999;pointer-events:none;">
    ...
    {{-- Modal Kecil --}}
    <div id="modalKecil" class="peta-modal fixed inset-0 z-[2000] hidden items-start justify-center pt-12 px-4" role="dialog" aria-hidden="true">
        <div class="fixed inset-0 bg-black/50" data-dismiss="modal"></div>
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm z-10 flex flex-col max-h-[85vh]">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
                <h4 class="text-sm font-semibold text-gray-800 modal-title"></h4>
                <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none font-bold bg-transparent border-0 cursor-pointer p-0" data-dismiss="modal">&times;</button>
            </div>
            <div class="fetched-data overflow-y-auto p-5 flex-1 text-sm"></div>
        </div>
    </div>

    {{-- Modal Sedang --}}
    <div id="modalSedang" class="peta-modal fixed inset-0 z-[2000] hidden items-start justify-center pt-12 px-4" role="dialog" aria-hidden="true">
        <div class="fixed inset-0 bg-black/50" data-dismiss="modal"></div>
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg z-10 flex flex-col max-h-[85vh]">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
                <h4 class="text-sm font-semibold text-gray-800 modal-title"></h4>
                <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none font-bold bg-transparent border-0 cursor-pointer p-0" data-dismiss="modal">&times;</button>
            </div>
    ...
    {{-- Modal Besar --}}
    <div id="modalBesar" class="peta-modal fixed inset-0 z-[2000] hidden items-start justify-center pt-12 px-4" role="dialog" aria-hidden="true">
        <div class="fixed inset-0 bg-black/50" data-dismiss="modal"></div>
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl z-10 flex flex-col max-h-[85vh]">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
                <h4 class="text-sm font-semibold text-gray-800 modal-title"><i class="fa fa-exclamation-triangle text-yellow-500 mr-1"></i></h4>
                <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none font-bold bg-transparent border-0 cursor-pointer p-0" data-dismiss="modal">&times;</button>
            </div>
            <div class="fetched-data overflow-y-auto p-5 flex-1 text-sm"></div>
        </div>
...
@push('scripts')
    <script>
    // Modal shim — menggantikan Bootstrap JS untuk peta modal
    // Tetap memicu event show.bs.modal / shown.bs.modal agar peta.js berfungsi
    (function($) {
        function openModal($modal, relatedTarget) {
            ...
            $modal.trigger($.Event('shown.bs.modal'));
        }
        function closeModal($modal) {
            $modal.addClass('hidden').removeClass('flex');
            $modal.find('.fetched-data').empty();
            document.body.style.overflow = '';
            $modal.trigger('hidden.bs.modal');
        }
        $.fn.modal = function(action) {
            return this.each(function() {
                if (action === 'show') openModal($(this), null);
        ...
        $(document).on('click', '[data-toggle="modal"]', function(e) {
            ...
            if ($modal.length) openModal($modal, this);
        });
        $(document).on('click', '[data-dismiss="modal"]', function() {
            closeModal($(this).closest('.peta-modal'));
        });
        $(document).on('keydown', function(e) {
            if (e.key === 'Escape') $('.peta-modal.flex').each(function() { closeModal($(this)); });
        });
    })(jQuery);
    </script>
